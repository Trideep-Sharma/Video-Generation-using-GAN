...
class VideoGAN_test():
    ...
    def train(self):
        ...
        saver.restore(self.sess,"./checkpoints/VideoGAN_1_995_107.ckpt")
        ...
        for epoch in range(self.epochs):
            for counter in range(len(data_files)/self.batch_size):
                ...
                gen_videos,bg = self.sess.run([self.genvideo,self.bg], feed_dict = {self.zsample : noise})
                print("....Iteration....:", counter,"of epoch....",str(epoch)+"/"+str(self.epochs))
                # print (str(i) + "/"+ str(iterations) + " Gen Loss: " + str(gLoss) + " Disc Loss: " + str(dLoss))
                ...
                # print ('video arr shape',videos.shape)
    ...
    #             print("Generator Loss", gloss)
                if np.mod(counter + 1, print_count) == 0:
                    process_and_write_video(gen_videos,"video" + str(counter)+str(epoch))
                    process_and_write_image(bg,"bg" + str(counter)+str(epoch))
            ...
            print ('Saved {}_{}'.format(counter,epoch))
    ...
    def visualize_videos(self):
        with tf.variable_scope("generator") as scope:
            ...
            #Incorporate mask
            video = tf.add(tf.multiply(mask,foreground),tf.multiply(1-mask,background))
            print("Video Shape (visualize_video fn)")
            print (video.get_shape())
